class ProjectsCard {
  constructor() {
    ...
    this.projects = {
      populationTracker: {
        ...
        description: 'Population Tracker is a data visualization tool, which fetches U.S. Census records to render a colorful map. Population is displayed by year, and broken down by state or territory.',
      ...
      yup: {
        ...
        description: 'Yup is a full stack Yelp clone, featuring User Auth and CRUD functionality.',
      ...
      nyght: {
        ...
        description: 'NYGHT is an evening/date planning app, which presents users with an itinerary for the night based on neighborhood and preference. Logged in users can save, edit, and share itineraries with others via email.',
        techStack: 'JavaScript, React and Redux, Express.js, and MongoDB.'
      }
    };

    const projectsContainer = this.render();
    document.querySelector('#projects-container').append(projectsContainer);
  }
  ...
  render() {
    ...
    const cssIfyName = (string) => {
      ...
      for (let i = 0; i < string.length; i++) {
        string[i].match(symbolFilter) ? (cssIfied += string[i].toLowerCase()) : (cssIfied += '-');
      }
      return cssIfied;
    };
    ...
    Object.values(this.projects).forEach((project, idx) => {
      ...
      projectItem.classList.add('project-item', cssIfyName(project.title));
  ...
  expandProjects(e) {
    e.preventDefault();
    if (!expanded) {
      projectItemsContainer.appendChild(render());

      expanded = true;
    } else {
      const projectItemsContent = document.querySelector('div.project-items-content');
